export async function seedDemoData(pool, userId) {
    ...
    try {
        ...
        for (const bean of beans) {
            const [result] = await conn.execute(`INSERT INTO beans (user, name, roaster, origin, variety, process, flavour_summary, roast_level, elevation_m, status)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, bean.name, bean.roaster, bean.origin, bean.variety, bean.process, bean.flavour_summary, bean.roast_level, bean.elevation_m, bean.status]);
        ...
        for (let i = 0; i < beanIds.length; i++) {
            ...
            if (!p || beanId === undefined)
                continue;
            await conn.execute(`INSERT INTO bean_palette (bean_id, pri_hue, sec_hue, acc_hue) VALUES (?, ?, ?, ?)`, [beanId, p.pri, p.sec, p.acc]);
        }
        // column: user (VARCHAR 50)
        const gear = [
        ...
        const gearIds = [];
        for (const g of gear) {
            const [result] = await conn.execute(`INSERT INTO gear (name, type, notes, user) VALUES (?, ?, ?, ?)`, [g.name, g.type, g.notes, userId]);
            gearIds.push(result.insertId);
        }
        // Seeded at signup by seedDefaultRecipes.
        const [recipeRows] = await conn.query(`SELECT id FROM recipes WHERE user = ? ORDER BY id LIMIT 5`, [userId]);
        ...
        for (const b of brews) {
            ...
            if (beanId === undefined || recipeId === undefined || brewerId === undefined || grinderId === undefined)
                continue;
            await conn.execute(`INSERT INTO brews (user, bean_id, recipe_id, brewer_id, grinder_id, grind_size, dose_g, yield_g, time_seconds, closeness, profile, body, notes, brewed_at)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, beanId, recipeId, brewerId, grinderId, b.grind, b.dose, b.yield, b.time, b.closeness, b.profile, b.body, b.notes, b.brewed_at]);
        }
        await conn.commit();
