...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const [[existing]] = await connection.query('SELECT id FROM beans WHERE id = ? AND user = ?', [beanId, userId]);
        if (!existing) {
            res.status(404).json({ error: 'Bean not found' });
            return;
        }
        // Update the bean row
        await connection.query(`UPDATE beans SET
					name = ?,
					roaster = ?,
					origin = ?,
					variety = ?,
        ...
        let flavourNote = (flavourNotes ?? '').split(',').map((n) => n.trim());
        ...
        await connection.query(`UPDATE bean_palette SET
					pri_hue = ?,
					sec_hue = ?,
					acc_hue = ?
				WHERE bean_id = ?`, [flavourHues[0], flavourHues[1], flavourHues[2], beanId]);
        res.json({ id: beanId });
    }
    catch (err) {
        console.error(err);
...
//# sourceMappingURL=editBean.js.map