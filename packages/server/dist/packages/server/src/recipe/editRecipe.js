...
router.put('/', requireAuth, async (req, res) => {
    try {
        ...
        try {
            ...
            const [result] = await conn.execute('UPDATE recipes SET name = ?, brew_method = ? WHERE id = ? AND user = ?', [name, brewMethod, id, userId]);
            ...
            for (const [i, step] of steps.entries()) {
                await conn.execute('INSERT INTO recipe_steps (recipe_id, step_order, action, duration_seconds) VALUES (?, ?, ?, ?)', [id, i + 1, step.action, step.duration ?? 0]);
            }
            await conn.commit();
