...
export async function seedDefaultRecipes(pool: Pool, userId: string) {
  ...
  try {
    ...
    for (const recipe of recipes) {
      const [result] = await conn.execute<any>(
        `INSERT INTO recipes (name, brew_method, default_dose_g, user)
         VALUES (?, ?, ?, ?)`,
        [recipe.name, recipe.brew_method, recipe.default_dose_g, userId]
      );
